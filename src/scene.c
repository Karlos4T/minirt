...
/*   Updated: 2024/04/08 17:22:12 by carlosga         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_scene	*initialize_scene()
{
	...
	scene->camera = start_camera(0, 100, 0, 0, 0, -1, 90);
	scene->alight = create_alight(0.3, 0x00FFFFFF);
	scene->lights = create_light(500, 0, -700, 1, 0x00FFFFFF);
	scene->objects = malloc(sizeof(t_objects));
	scene->objects->cylinders = malloc(sizeof(t_cylinder) * 2);
	scene->objects->cylinders[0] = create_cylinder(0, 0, -400, -1, 0, 1, 40, 100, 0x0000FFFF);
	scene->objects->cylinders[1] = create_cylinder(0, 200, -400, 0, 1, -1, 40, 80, 0x0000FFFF);
	scene->objects->cylinders[2] = create_cylinder(200, 200, -400, 0, 1, 0, 100, 100, 0x0000FFFF);
	...
	scene->objects->planes[1] = create_plane(0, 0, -800, 0, 0, 1, 0x00177717);
